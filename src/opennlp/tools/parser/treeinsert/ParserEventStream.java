...
public class ParserEventStream extends AbstractParserEventStream {
  ...
  private int nonPunctChildCount(Parse node) {
    return AbstractBottomUpParser.collapsePunctuation(node.getChildren(),punctSet).length;
  }
  /*
  ...
  */
  ...
  protected void addParseEvents(List<Event> parseEvents, Parse[] chunks) {
    ...
    for (int ci=0;ci<chunks.length;ci++) {
      ...
      currentChunks[ci].setLabel(AbstractBottomUpParser.COMPLETE);
      chunks[ci].setLabel(AbstractBottomUpParser.COMPLETE);
    }
    for (int ci=0;ci<chunks.length;ci++) {
        ...
        if (parent.getLabel() == null && !prevParent.getType().equals(parent.getType())) {
          ...
          if (lastChild(chunks[ci], parent)) {
            if (etype == ParserEventTypeEnum.CHECK) {
              parseEvents.add(new Event(AbstractBottomUpParser.COMPLETE, checkContextGenerator.getContext(currentChunks[ci],currentChunks, ci,false)));
            }
            currentChunks[ci].setLabel(AbstractBottomUpParser.COMPLETE);
            parent.setLabel(AbstractBottomUpParser.COMPLETE);
          }
          else {
            if (etype == ParserEventTypeEnum.CHECK) {
              parseEvents.add(new Event(AbstractBottomUpParser.INCOMPLETE, checkContextGenerator.getContext(currentChunks[ci],currentChunks,ci,false)));
            }
            currentChunks[ci].setLabel(AbstractBottomUpParser.INCOMPLETE);
            parent.setLabel(AbstractBottomUpParser.COMPLETE);
          }
      ...
      if (ci == 0){
        ...
        for (int cfi=0;cfi<currentRightFrontier.size();cfi++) {
          ...
          if (!Parser.checkComplete || !AbstractBottomUpParser.COMPLETE.equals(cfn.getLabel())) {
          ...
          if (Parser.checkComplete && cfn.getLabel().equals(AbstractBottomUpParser.INCOMPLETE)) {
        ...
        if (attachNode != null) {
          if (attachType == Parser.ATTACH_DAUGHTER) {
            ...
            if (lastChild(chunks[ci], rightFrontier.get(attachNodeIndex))) {
              if (etype == ParserEventTypeEnum.CHECK) {
                parseEvents.add(new Event(AbstractBottomUpParser.COMPLETE, checkContextGenerator.getContext(attachNode,currentChunks,ci,true)));
              }
              attachNode.setLabel(AbstractBottomUpParser.COMPLETE);
            }
            else {
              if (etype == ParserEventTypeEnum.CHECK) {
                parseEvents.add(new Event(AbstractBottomUpParser.INCOMPLETE, checkContextGenerator.getContext(attachNode,currentChunks,ci,true)));
              }
            }
          }
            ...
            if (lastChild(chunks[ci], rightFrontier.get(attachNodeIndex))) {
              if (etype == ParserEventTypeEnum.CHECK) {
                parseEvents.add(new Event(AbstractBottomUpParser.COMPLETE, checkContextGenerator.getContext(newParent,currentChunks,ci,true)));
              }
              newParent.setLabel(AbstractBottomUpParser.COMPLETE);
            }
            else {
              if (etype == ParserEventTypeEnum.CHECK) {
                parseEvents.add(new Event(AbstractBottomUpParser.INCOMPLETE, checkContextGenerator.getContext(newParent,currentChunks,ci,true)));
              }
              newParent.setLabel(AbstractBottomUpParser.INCOMPLETE);
            }
