...
public String getHelp() {
  ...
  public void run(InputStream modelIn,String line, PrintWriter printWriter) {
    
    if (true) {
    
      ParserModel model = new ParserModelLoader().load(modelIn);

      Integer beamSize = null;//CmdLineUtil.getIntParameter("-bs", args);
      if (beamSize == null)
          beamSize = AbstractBottomUpParser.defaultBeamSize;

      Integer numParses = null;//CmdLineUtil.getIntParameter("-k", args);
      boolean showTopK;
      if (numParses == null) {
        numParses = 1;
        showTopK = false;
      ...
      Double advancePercentage = null;//CmdLineUtil.getDoubleParameter("-ap", args);
