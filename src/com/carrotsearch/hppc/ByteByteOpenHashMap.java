...
/**
 * A hash map of <code>byte</code> to <code>byte</code>, implemented using open
 ...
 */
 @javax.annotation.Generated(date = "2014-09-08T10:42:29+0200", value = "HPPC generated from: ByteByteOpenHashMap.java") 
public class ByteByteOpenHashMap
    implements ByteByteMap, Cloneable
{
    /**
     ...
     */
    public ByteByteOpenHashMap(int initialCapacity)
    {
     ...
     */
    public ByteByteOpenHashMap(int initialCapacity, float loadFactor)
    {
     ...
     */
    public ByteByteOpenHashMap(ByteByteAssociativeContainer container)
    {
    ...
    @Override
    public int putAll(
        ByteByteAssociativeContainer container)
    {
        final int count = this.assigned;
        for (ByteByteCursor c : container)
        {
            put(c.key, c.value);
        }
    ...
    @Override
    public int putAll(
        Iterable<? extends ByteByteCursor> iterable)
    {
        final int count = this.assigned;
        for (ByteByteCursor c : iterable)
        {
            put(c.key, c.value);
        }
        return this.assigned - count;
     ...
     */
         ...
         public byte putOrAdd(byte key, byte putValue, byte additionValue)
    {
        assert assigned < allocated.length;
     ...
     */
         ...
         public byte addTo(byte key, byte additionValue)
    {
     ...
     */
    private void expandAndPut(byte pendingKey, byte pendingValue, int freeSlot)
    {
     ...
     */
    private final class EntryIterator extends AbstractIterator<ByteByteCursor>
    {
        private final ByteByteCursor cursor;

        public EntryIterator()
        {
            cursor = new ByteByteCursor();
            cursor.index = -1;
        }

        @Override
        protected ByteByteCursor fetch()
        {
     ...
     */
    @Override
    public <T extends ByteByteProcedure> T forEach(T procedure)
    {
        final byte [] keys = this.keys;
        final byte [] values = this.values;
        final boolean [] states = this.allocated;
    ...
    {                                                                                                                                         
        ...
        public int size()                                                                                                                     
        {                                                                                                                                     
            return ByteByteOpenHashMap.this.size();                                                                                       
        }                                                                                                                                     
                                                                                                                                              
        @Override                                                                                                                             
        public boolean isEmpty()                                                                                                              
        {                                                                                                                                     
            return ByteByteOpenHashMap.this.isEmpty();                                                                                    
        }                                                                                                                                     
        ...
        {                                                                                                                                     
            ...
            final boolean [] allocated = ByteByteOpenHashMap.this.allocated;                                                              
            final byte [] values = ByteByteOpenHashMap.this.values;                                                                      
     ...
     */
    @Override
    public ByteByteOpenHashMap clone()
    {
        try
        {
            /*  */
            ByteByteOpenHashMap cloned = 
                (ByteByteOpenHashMap) super.clone();
    ...
    public String toString()
    {
        ...
        boolean first = true;
        for (ByteByteCursor cursor : this)
        {
     ...
     */
    public static  ByteByteOpenHashMap from(byte [] keys, byte [] values)
    {
        if (keys.length != values.length) 
            throw new IllegalArgumentException("Arrays of keys and values must have an identical length."); 

        ByteByteOpenHashMap map = new ByteByteOpenHashMap();
     ...
     */
    public static  ByteByteOpenHashMap from(ByteByteAssociativeContainer container)
    {
        return new ByteByteOpenHashMap(container);
    }
     ...
     */
    public static  ByteByteOpenHashMap newInstanceWithoutPerturbations()
    {
        return new ByteByteOpenHashMap() {
            @Override
     ...
     * shortcut). 
     */
    public static  ByteByteOpenHashMap newInstance(int initialCapacity, float loadFactor)
    {
        return new ByteByteOpenHashMap(initialCapacity, loadFactor);
    }
     ...
     */
    public static  ByteByteOpenHashMap newInstanceWithExpectedSize(int expectedSize)
    {
     ...
     * <code>expectedSize</code> elements without having to resize.
     */
    public static  ByteByteOpenHashMap newInstanceWithExpectedSize(int expectedSize, float loadFactor)
    {
