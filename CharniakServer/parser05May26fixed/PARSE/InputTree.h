...
class  InputTree
{
 public:
  ...
  const ECString head() const { return headTree_->word(); } //Vasin
  const ECString hTag() const { return headTree_->term(); } //Vasin
  InputTrees& subTrees() { return subTrees_; } //Vasin
  const InputTrees& subTrees() const { return subTrees_; } //Vasin
