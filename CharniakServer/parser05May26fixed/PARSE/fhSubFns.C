...
extern AnsTree* curAnsTree;
int nullWordInt;
AnsTree*  tree_ruleTree(FullHist* treeh, int ind);
...
{
  AnsTree* spectree = NULL;
  if(Feature::isLM) spectree = tree_ruleTree(tree, 2);
  if(spectree) return spectree->wrd;
...
tree_ruleHead_third(FullHist* treeh)
{
  ...
  AnsTree* trdtree = tree_ruleTree(treeh,3);
